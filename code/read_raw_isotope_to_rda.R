...
iso <- read_xlsx(here("data", "TilcockM SutterCNTray1 CN 0520 (1).xlsx"),
                 sheet = "Samples",
                 range = cell_cols("A:J"))

# this is the same but without "here", still using relative link though!
iso <- read_xlsx("data/TilcockM SutterCNTray1 CN 0520 (1).xlsx",
                 sheet = "Samples",
                 range = cell_cols("A:J"))
...
# this is the same but without "here", still using relative link though!
iso_meta <- read_xlsx("data/TilcockM SutterCNTray1 CN 0520 (1).xlsx",
                 sheet = "Summary",
                 range = "A22:C23",
...
# use glue to add submission date to filename and save as .rdata file (.rda)
library(glue)

save(iso_clean, file = glue("data_output/{Sys.Date()}_iso_tidy_submitted_{iso_meta[1,3]}.rda"))

# save as a csv
write_csv(iso_clean, file=glue("data_output/{Sys.Date()}_iso_tidy_submitted_{iso_meta[1,3]}.csv"))
